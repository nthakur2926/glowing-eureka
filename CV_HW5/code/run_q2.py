...
for itr in range(max_iters):
    ...
    for xb,yb in batches:
         h1 = forward(xb, params, 'layer1')  # forward
         probs = forward(h1, params, 'output', softmax) # loss
         loss, acc = compute_loss_and_acc(yb, probs)# be sure to add loss and accuracy to epoch totals
    ...
    if v.ndim != 1:
        for row in range(0,height):
                for col in range(0,v.shape[1]):
                        ...
                        v[row, col] = v_
                        subtract = (loss_1 - loss_2)
                        fraction = subtract/(2 * 1e-6)    
                        grad_v[row, col] = subtract
